...
$(function() {
  ...
  let resultArr = [];
  // DOM設定
  const $area = $('select[id="area"]');
  const $pref = $('select[id="prefecture"]');
  const $city = $('select[id="city"]');
  ...
  $("#datepicker").datepicker({
    ...
    const city = $city.val();
    // 選択日付
    const date = $("#datepicker").val().replaceAll('/', '').replaceAll('-', '');
  ...
  $pref.change(function() {
    ...
    for (let pref of defCity) { 
      ...
      if (pref[0] == prefName) {
        ...
        // 元データ市町村ループ
        for (let city of pref[1]) {
          // 市町村を追加
          $city.append(`<option value="${city}" data-val="${pref[0]}">${city}</option>`);
        }
      }
    }
    // 実行
